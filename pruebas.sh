...
comparar() {
	...
	echo -n "$2.in.txt: "
	if "$java"; then
		java Ejercicio"$1" <Pruebas/Ejercicio"$1"/"$2".in.txt >Pruebas/Ejercicio"$1"/"$2".out.mine.txt || {
			echo "Error de ejecución"
			return 1
		}
	else
		./ejercicio"$1".out <Pruebas/Ejercicio"$1"/"$2".in.txt >Pruebas/Ejercicio"$1"/"$2".out.mine.txt || {
			echo "Error de ejecución"
			return 1
		}
	fi
